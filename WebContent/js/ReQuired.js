    ...
    if (IsEmpet(content)) {
        if (Length(content, length)) {
            ...
            if (!myRegh.test(content.value)) {
                ShowError(content.id, "<font color='red'>只能输入由数字和 26 个英文字母组成的字符串！</font>");
                return false;
            }

            //匹配表达式
            if (!myReg.test(content.value)) {
                ShowError(content.id, "<font color='red'>不能含有非法字符！</font>");
                return false;
            }
            else {
                ShowError(content.id);
                return true;
            }
        }
    }
...
function Length(content, num) {
    ...
    if (content.value.toString().length > parseInt(num)) {
        //如果为空显示提示信息
        ShowError(content.id, "<font color='red'>超出规定长度" + num + "！</font>");
        return false;
    }
    else {
        ShowError(content.id);
...
function Age(content, begin, end) {
    if (ZNum(content)) {
        ...
        if (age > endAge || age < startAge) {
            ShowError(content.id, "<font color='red'>年龄有问题！</font>");
            return false;
        }
        else {
            ShowError(content.id);
...
function DropDownSelect(content) {
    if (content.value.lastIndexOf('-') < 0) {
        ...
        ShowError(content.id, "<font color='red'>请选择合法的项！</font>");
        return false;
    }
}
//为空验证
function IsEmpet(content) {
    //判断是否为空
    if (content.value.toString().length == 0) {
        //如果为空显示提示信息

        ShowError(content.id, "<font color='red'>不能为空！</font>");
        return false;
    }
    else {
...
function Phone(content) {
    ...
    //匹配表达式
    if (!myReg.test(content.value)) {
        ShowError(content.id, "<font color='red'>电话号码格式不正确！</font>");
        return false;
    }
    else {
        ShowError(content.id);
...
function EName(content, num) {
    if (Length(content, num)) {
        ...
        var myReg = /^[A-Za-z]+$/;
        if (!myReg.test(content.value)) {
            ShowError(content.id, "<font color='red'>只能输入 26 个英文字母组成的字符串！</font>");
            return false;
        }
        else {
            ShowError(content.id);
...
function Fox(content) {
    ...
    //匹配表达式
    if (!myReg.test(content.value)) {
        ShowError(content.id, "<font color='red'>传真格式不正确！</font>");
        return false;
    }
    else {
        ShowError(content.id);
...
function YB(content) {
    ...
    var myReg = /^\d{6}/;
    if (!myReg.test(content.value)) {
        ShowError(content.id, "<font color='red'>邮编格式不正确！</font>");
        return false;
    }
    else {
        ShowError(content.id);
...
function Bank(content) {
    ...
    if (IsEmpet(content)) {
        ...
        var myReg = /^(?:[\u4e00-\u9fa5]*\w*\s*)+$/;
        if (!myReg.test(content.value)) {
            ShowError(content.id, "<font color='red'>开户银行不能含有特殊字符！</font>");
            return false;
        }
        else {
            ShowError(content.id);
...
function PWD(content) {
    ...
    if (!myReg.test(content.value)) {
        ShowError(content.id, "<font color='red'>长度在6～18之间！</font>");
        return false;
    }
    else {
        ShowError(content.id);
...
function RePWD(content, pwdid) {
    ...
    if (!myReg.test(content.value)) {
        ...
        if (pwdcontent.toString() != pwdrContent.toString()) {
            ShowError(content.id, "<font color='red'>两次密码不一致！</font>");
            return false;
        }
        else {
            ShowError(content.id);
...
function Num(content) {
    ...
    if (IsEmpet(content)) {
        ...
        var myReg = /^^\d*$/;
        if (!myReg.test(content.value)) {
            ShowError(content.id, "<font color='red'>只能输入数字！</font>");
            return false;
        }
        else {
            ShowError(content.id);
...
//正整数验证
function ZNum(content) {
    //正整数规则正则表达式
    var myReg = /^\+?[1-9][0-9]*$/;
    if (!myReg.test(content.value)) {
        ShowError(content.id, "<font color='red'>只能输入非零的正整数！</font>");
        return false;
    }
    else {
        ShowError(content.id);
...
function Line(content) {
    ...
    if (!myReg.test(content.value)) {
        ShowError(content.id, "<font color='red'>网址格式不正确！</font>");
        return false;
    }
    else {
        ShowError(content.id);
...
function Email(content) {
    ...
    if (!myReg.test(content.value)) {
        ShowError(content.id, "<font color='red'>邮箱格式不正确！</font>");
        return false;
    }
    else {
        ShowError(content.id);
...
function ID(content) {
    ...
    if (!myReg.test(content.value)) {
        ShowError(content.id, "<font color='red'>身份证格式不正确！</font>");
        return false;
    }
    else {
        ShowError(content.id);
...
function UpFile(content, compare) {
    ...
    if (content.value.toString().length != 0) {
        ...
        if (!tag) {
            ShowError(content.id, "<font color='red'>上传文件格式不正确,必须是" + compare + "格式！</font>");
            return false;
        }
        else {
            ShowError(content.id);
...
function ShowError(id, content) {
    if (typeof (content) != typeof (undefined)) {
        return $('#' + id + 'Error').html(content);
    }
    else {
        return $('#' + id + 'Error').html("<img alt='' src='../Images/Right.gif' />");
    }
}
//表单事件
function Formsubmit() {
