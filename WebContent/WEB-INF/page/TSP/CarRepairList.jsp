            ...
            if (flag == false) {
                ...
                if (confirm("确认删除吗？")) {
                	...
                	var chk1 = document.getElementsByName("chk");
                  	...
                  	$.ajax({
        				url : "CarRepairAction_delect.action",
        				type : "POST",
        				//data:{}如果没有就不传当有值的时候是比如：  
        				data:{"ids":ids},
        				dataType : "json",
        				complete : function() {
        					window.location.href = "/LIS/TSP/CarRepairAction_list.action";
        				}
        			});
            ...
            if (flag == false) {
                return false;
                 ...
                 for (var i = 0; i < chk1.length; i++) {
                     if (chk1[i].checked) {
                      id=chk1[i].value;  
                      window.location.href="/LIS/TSP/CarRepairAction_editUI.action?id="+id;
                     }
                 }
...
<body>
    ...
    <s:form action="CarRepairAction_query">
            ...
            <table class="tabModel">
  	            <tr>
			        ...
			        <td >	
  			            ...
  			            <s:select list="#{1:'',2:'重庆',3:'成都',4:'北京'}" listKey="value" listValue="value" name="repairPlace"></s:select>
      ...
      <!-- list-start -->
        ...
        <div class="divBtn" id="divBtn">
           <input type="button" value="新 增" class="btn" onclick="location.href='/LIS/TSP/CarRepairAction_addUI.action'"/>&nbsp;
            <%-- <s:a action="CarRepairAction_addUI" cssClass="btn">新增</s:a> --%>
    		    ...
    		    <tr>
    		     <td style="text-align:center;"><input type="checkbox" name="chk" value="${id }"/></td>
    			    ...
    			    <td>
    				    ...
    				    <s:a action="CarRepairAction_detail?id=%{id}">${id}</s:a>
    			    </td>
    			    <td>
    				    ${car.id}
    			    </td>
                           ...
                           尾页 </a>&nbsp; 转到第<input type="text" class="inputPage" />页 <a href="javascript:void(0)"
                             ...
                             <!-- 这是所有数据的list的分页，用一个变量标记 -->
                              <s:if test="%{#allListid=='allListid'}">
                              	...
                              	<s:else>
                              		<s:a action="CarRepairAction_list?pageBean.page=1" cssClass="btnPage">首页</s:a>
                              		<s:a action="CarRepairAction_list?pageBean.page=%{#pageBean.page-1}" cssClass="btnPage">上一页</s:a>
                              	</s:else>
                              	<s:if test="%{#pageBean.page==#pageBean.totalPage}">
                              		
                              	</s:if>
                              	<s:else>
                              		<s:a action="CarRepairAction_list?pageBean.page=%{#pageBean.page+1}" cssClass="btnPage">下一页</s:a>
                              		<s:a action="CarRepairAction_list?pageBean.page=%{#pageBean.totalPage}" cssClass="btnPage">最后一页</s:a>
                              	</s:else>
                              	&nbsp; 转到第
                              	<s:form action="CarRepairAction_list" cssStyle="display:block;float:right;margin-top:-19px">
                              ...
                              <s:else>
                              	...
                              	<s:else>
                              		<s:a action="CarRepairAction_query?pageBean.page=1&cahxuanid=1" cssClass="btnPage"> 首页</s:a>
                              		<s:a action="CarRepairAction_query?pageBean.page=%{#pageBean.page-1}&cahxuanid=1" cssClass="btnPage">上一页</s:a>
                              	</s:else>
                              	<s:if test="%{#pageBean.page==#pageBean.totalPage}">
                              	...
                              	&nbsp; 转到第
                              	<s:form action="CarRepairAction_query" cssStyle="display:block;float:right;margin-top:-19px">
                              	<%-- <input type="text" class="inputPage" />页 <s:a 
                                        id="btnGo" cssClass="btnPage">GO</s:a> --%>
                                        <s:textfield name="pageBean.page" cssClass="inputPage" ></s:textfield>
